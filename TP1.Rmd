...
Database<- read.csv("../data/GUIDE.csv")
```
...
if ("IncidentGrade_negatif" %in% colnames(df_copie)) {
  df_copie <- subset(df_copie, select = -IncidentGrade_negatif)
  df_copie <- df_copie %>%
...
if (!is.factor(df_copie$IncidentGrade)) {
...
names(df_copie) <- make.names(names(df_copie))

if (!is.factor(df_copie$IncidentGrade)) {
  df_copie$IncidentGrade <- factor(df_copie$IncidentGrade)
}
...
Avant de déployer le modèle, on doit d'abord préparer la base de données.


### Pré traitement de la Base des données : 

```{r Preparation-base-donnee, eval=FALSE,include=FALSE}
col_a_supprimer = c("Id", "OrgId" ,"IncidentId","AlertId","DetectorId" ,
                    "DeviceId","AccountSid","AccountObjectId",
                    "NetworkMessageId","EmailClusterId","ApplicationId",
                    "OAuthApplicationId","ResourceIdName", "ActionGrouped", 
                    "ActionGranular", "ResourceType","ThreatFamily",
                    "AntispamDirection", "Roles")
for (col in  col_a_supprimer){
  Database[[col]] <- NULL
}

Database$SuspicionLevel[is.na(Database$SuspicionLevel)] <- "Not Suspicious"
Database$LastVerdict[is.na(Database$LastVerdict)] <- "Pending Analysis"
Database$MitreTechniques[is.na(Database$MitreTechniques)] <- "Unknown"
...
data1 <- Database[, colnames(df_10_et_moins)]
data_onehot <- dummy_cols(data1, remove_first_dummy = FALSE, remove_selected_columns = TRUE)

data2 <- Database[, colnames(df_plus_10)]
data_encoded <- encode_frequencies(data2)


data <- cbind(data_onehot, data_encoded)
data <- data[, sapply(data, is.numeric)]

if ("IncidentGrade_negatif" %in% colnames(data)) {
  data <- subset(data, select = -IncidentGrade_negatif)
  data <- data %>%
  rename(IncidentGrade = IncidentGrade_positif)
}


if (!is.factor(data$IncidentGrade_NA)) {
  data$IncidentGrade <- factor(data$IncidentGrade_NA, levels = c(0, 1), labels = c("negatif", "positif"))
  data <- subset(data, select = -IncidentGrade_NA)
}

glimpse(data)

```
```{r preparation-des-donnees-2}
...
col_a_supprimer <- c("Id", "OrgId" ,"IncidentId","AlertId","DetectorId" ,
                     ...
                     "ActionGranular", "ResourceType","ThreatFamily",
                     "AntispamDirection", "Roles")
Database <- Database[, !(names(Database) %in% col_a_supprimer)]
...
cat_vars <- sapply(Database, is.character) | sapply(Database, is.factor)
categorical_df <- Database[, cat_vars]


nb_modalites <- sapply(categorical_df, function(x) length(unique(x)))

df_10_et_moins <- categorical_df[, nb_modalites <= 10]
df_plus_10 <- categorical_df[, nb_modalites > 10]


if (ncol(df_10_et_moins) > 0) {
  data_onehot <- dummy_cols(df_10_et_moins, remove_first_dummy = FALSE, remove_selected_columns = TRUE)
} else {
  data_onehot <- data.frame()
  warning("Aucune variable avec ≤10 modalités.")
}

encode_frequencies <- function(df) {
  for (col in names(df)) {
    freq_table <- table(df[[col]]) / nrow(df)
    df[[col]] <- as.numeric(freq_table[df[[col]]])
  }
  return(df)
}

if (ncol(df_plus_10) > 0) {
  data_encoded <- encode_frequencies(df_plus_10)
} else {
...
# Nettoyage IncidentGrade (OneHot → variable cible)
if ("IncidentGrade_negatif" %in% colnames(data)) {
  data <- subset(data, select = -IncidentGrade_negatif)
  data <- data %>% rename(IncidentGrade = IncidentGrade_positif)
}

if ("IncidentGrade_NA" %in% colnames(data)) {
  data$IncidentGrade <- factor(data$IncidentGrade_NA, levels = c(0, 1), labels = c("negatif", "positif"))
  data <- subset(data, select = -IncidentGrade_NA)
}
...
```{r  deploiement-du-modele-choisit, include=FALSE, eval=FALSE}
predictions_reelles <- predict(arbre_elague, newdata = data, type = "class")

# Matrice de confusion et précision
confusion <- table(Prediction = predictions_reelles, Réel = data$IncidentGrade)
print(confusion)
accuracy <- mean(prediction == data$IncidentGrade)
